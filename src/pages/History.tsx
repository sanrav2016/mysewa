    ...
    <div className="space-y-6 p-4 lg:p-8">
      ...
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-6 rounded-2xl shadow-lg border-4 border-orange-200 dark:border-slate-600">
        <div className="flex items-start md:items-center gap-4 flex-col md:flex-row">
          <div className="w-12 h-12 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl flex items-center justify-center text-white font-bold shrink-0">
            {targetUser.name.charAt(0)}
          </div>
          <div className="space-y-2">
            <h1 className="text-3xl font-bold text-slate-800 dark:text-white">
              {targetUser.name}'s History
            </h1>
          ...
          <div className="flex flex-wrap gap-2">
            {(['all', 'confirmed', 'waitlist', 'cancelled'] as const).map((status) => (
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-xl font-medium transition-colors ${stickyControls ? "text-sm" : "text-base"} ${filter === status
      ...
      {/* Events Table */}
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-lg border-4 border-orange-200 dark:border-slate-600 w-full max-w-screen">
                        ...
                        >
                          <td className="py-3 px-2 sm:px-4">
                            <div className="flex items-center gap-2 sm:gap-3">
                              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0">
                              ...
                              <div className="min-w-0 flex-1">
                                <Link className="transition-colors dark:text-white text-slate-800 hover:text-orange-600 dark:hover:text-orange-400" to={`/events/${eventData.eventId}`}>
                                  <span className="font-medium truncate">{eventData.event.title}</span>
                                </Link>
                                <div className="text-sm text-slate-500 dark:text-slate-400 truncate">{eventData.event.category}</div>
                              </div>
                            </div>
                          </td>
